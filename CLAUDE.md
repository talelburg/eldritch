...
**The publisher's own FAQ document is vendored too: `data/official-faq/`.** Read `data/rules-reference/rules/` first — it is indexed per entry, and it already contains the FAQ's complete numbered rules sections, so it answers most procedural questions on its own. Go to the Official FAQ when it doesn't, and in particular to `data/official-faq/Frequently_Asked_Questions.md`: its 145 Q&A pairs appear in **no other vendored source**, and a rule that is "missing" from the rules reference is very often answered there. #672 was filed because that lookup had nowhere to go. **The Official FAQ wins on conflict** — *"the text of this document takes precedence"* over the Rules Reference — and **absence there is a real answer**, not a gap: if neither source has it, say so rather than reconstructing it. `data/official-faq/README.md` indexes every section and every question.

When implementing or citing **rules behavior** — ability timing, trigger windows, framework events, skill-test resolution, action structure, anything procedural — verify before asserting; don't trust memory. **Read `data/rules-reference/rules/`**, ArkhamDB's rules reference ingested verbatim: the printed Rules Reference plus the rules added by deluxe expansions and by the official FAQ, one small file per section and per glossary entry, indexed by [`data/rules-reference/rules/README.md`](data/rules-reference/rules/README.md). Filenames are ArkhamDB anchor ids, so an old reference to `#Skill_Test_Timing` names its own file. Read the whole entry — every file is small enough to. Quote the load-bearing clause verbatim in PR descriptions and engine doc-comments where the rule shapes behavior; elision is fine for decorative surrounding clauses, but never substitute words. Lower-tier mirrors (Rulepop, fan wikis) lag and sometimes disagree — avoid them, and see **Rules text** in `CONTEXT.md` for why the *rules reference* PDF is provenance only. (The official FAQ PDF under `data/official-faq/` is a different case — it is the current edition, and the markdown beside it is converted from it.) When the user asks for a rules-based judgment call, the citation belongs in the answer.

**If the answer genuinely isn't in the vendored text, stop and say so.** Don't fetch it, and don't reconstruct it. A real gap means the data is stale or the question was misunderstood, and both want the maintainer's attention — a network fetch papers over exactly the problem the vendoring solves. Provenance and refresh procedures: [`data/rules-reference/SOURCE.md`](data/rules-reference/SOURCE.md), [`data/arkhamdb-faq/SOURCE.md`](data/arkhamdb-faq/SOURCE.md) and [`data/official-faq/SOURCE.md`](data/official-faq/SOURCE.md).
