...
- **The tag must match the quoted word.** Every card module opens with the printed text verbatim; that block is the evidence, and reading it against the `EventTiming` twenty lines below is a named review step (`docs/agents/standards.md`). *When* interrupts the condition, *at* and *if* land between, *after* waits until it has fully resolved.
...
**There is no automated check, and rebuilding one is not the fix.** The trigger word is not mechanically derivable from the printed text: *"if … would …"* is when-tier while a bare *"if"* on a settled state is at-tier, and *"would"* outranks the leading word entirely (`glossary/Instead.md`). A parser would encode one reading of the rules and then be trusted as though it had checked them. The six mis-tags #694 audited were not a gap in the evidence — every one of those modules quoted its own trigger word directly above the wrong enum. The failure was an unassigned reading, which is what the standards row assigns.
