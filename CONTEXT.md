...
_Avoid_: Treating the trigger word as flavour, and confusing a cell with the triggering condition it hangs off — one condition has all three cells, every time it occurs. Six of the nineteen timing declarations in the shipped corpus name the wrong cell (#694), all of them written from a guess rather than from the card.
