...
use crate::state::{
    CardCode, CardInstanceId, EnemyId, GameState, Investigator, InvestigatorId, Phase, WindowKind,
};
...
fn start_scenario(state: &mut GameState, events: &mut Vec<Event>) -> EngineOutcome {
    ...
    for inv_id in inv_ids {
        cards::shuffle_player_deck(state, events, inv_id);
        cards::draw_cards(state, events, inv_id, cards::INITIAL_HAND_SIZE);
    }
...
fn mythos_phase(state: &mut GameState, events: &mut Vec<Event>) {
    ...
    act_agenda::place_doom_on_agenda(state, events);

    // 1.3 Check doom threshold.
    act_agenda::check_doom_threshold(state, events);
...
fn upkeep_draw_and_resource(state: &mut GameState, events: &mut Vec<Event>) {
    ...
    for &id in &ids {
        cards::draw_one_with_deckout(state, events, id);
    }
    for &id in &ids {
        cards::grant_resources(state, events, id, 1);
    }
}
