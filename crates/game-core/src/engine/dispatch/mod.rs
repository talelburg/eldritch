...
use crate::state::{
    CardCode, CardInPlay, CardInstanceId, DefeatCause, Enemy, EnemyId, GameState, HunterChoice,
    Investigator, InvestigatorId, LocationId, Phase, SpawnEngagePending, Status, WindowKind, Zone,
};
...
/// with simultaneous-placement semantics (i.e. [`enemy_attack`](combat::enemy_attack) and
/// any future card effect that deals both) compose the lower-level
/// [`apply_damage_numeric`](combat::apply_damage_numeric) + [`apply_horror_numeric`](combat::apply_horror_numeric) +
...
fn take_horror(
    ...
    if combat::apply_horror_numeric(state, events, investigator, amount) {
