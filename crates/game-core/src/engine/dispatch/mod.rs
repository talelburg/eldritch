...
use crate::dsl::{Cost, EventPattern, EventTiming, SkillTestKind, Trigger};
use crate::event::Event;
use crate::state::{
    CardCode, CardInPlay, CardInstanceId, DefeatCause, Enemy, EnemyId, FastActorScope,
    FinishContinuation, GameState, HunterChoice, Investigator, InvestigatorId, LocationId,
    OpenWindow, PendingTrigger, Phase, SkillKind, SkillTestFollowUp, SpawnEngagePending, Status,
    WindowKind, Zone,
};
...
fn close_reaction_window_at(
    ...
    if let Some(in_flight) = state.in_flight_skill_test.as_ref() {
        if !matches!(in_flight.continuation, FinishContinuation::AwaitingCommit) {
            return skill_test::drive_skill_test(state, events);
        }
    }
