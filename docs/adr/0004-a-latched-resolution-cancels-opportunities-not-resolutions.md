...
*Status: decided, not yet built. This records the design agreed for #566; the `ScenarioEnd` frame and the drive-loop gate it describes land with that issue's PR. It rides #569's branch because the two were one design session, and #569 ships first (its contract — [ADR 0003](0003-emitting-a-timing-point-queues-abilities.md) — is what this builds on).*
