...
The reshape exposed a fidelity gap it deliberately did not close. Agenda 01107's reverse is **conditional** — `(→R3)` only *"If the investigators are at Act 1 or 2"*, while the act-3 branch prints no resolution point at all and instead defeats everyone who has not resigned. `Agenda.resolution` is printed data rather than effect DSL, so it cannot read `act_index` to choose; the act-1/2 branch ships with a `# Module gap` note and #806 tracks the rest. Under `Lost { reason }` that approximation was merely vague. Typed, it is a wrong id — which is the cost of making the id load-bearing, and the reason the gap had to be written down rather than left implicit.
