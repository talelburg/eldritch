...
**That answer forces one `Condition` variant, against `standards.md`'s one-card threshold.** 01115's gate has to be answerable with **no** "you" bound, and the escape hatch cannot be: a `Condition::Native` predicate is `fn(&GameState, &EvalContext) -> bool`, and an `EvalContext` names a controller, so a native cannot be asked anything at all on behalf of an uncontrolled card. `Condition::CardControlledByAPlayer { code }` is board-global by construction, and `Condition::Not` carries the printed negation rather than folding it into a `bool` field that would not compose. The threshold exists to stop the vocabulary being fixed around a sample of one; it does not license a shape that cannot express the card.
