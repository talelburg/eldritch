...
`abilities_in_effect::for_source` — the funnel #774 already built, through which **every** reader asks what abilities a card has — returns the side-in-effect printed abilities plus whatever the sweep grants. The sweep is the walk `modified_value::sweep` performs over all seven in-play collections, matching `Effect::Grant` where that one matches `Effect::Modify`; and that sweep reads the funnel too, so a granted bare `Effect::Modify` reaches stats exactly as a printed one does. Two costs, both recorded on `sweep`: the walk is quadratic, since the funnel runs its own board walk per source inside the walk over every source; and a grant's condition is evaluated during a modifier read, which could re-enter the sweep through `eval_condition` — no corpus grant does, since `Condition::ControlStatus` reads no modified quantity. The funnel has a registry-taking core (`for_source_with`) beneath its global-reading wrappers, because a modified value is a pure query over `(state, registry)` and takes its registry by argument.
...
*Folded #773. The composition claim above was originally about enumerators only; #773 found `modified_value::sweep` reading `abilities_for` directly, so the ADR's own claim — a granted ability is an ability — had not reached the modifier reader.*
