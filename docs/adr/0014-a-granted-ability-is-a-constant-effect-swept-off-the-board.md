...
**The two sweeps compose (#773).** `abilities_in_effect::for_source` is the funnel for every *ability* path, and `modified_value::sweep` was not one of them: it read `abilities_for` directly, so a granted bare `Effect::Modify` was merged into the recipient's ability list and reached no stat. That is not a second decision — it is this ADR's own claim, that a granted ability **is** an ability, applied to the one reader that had not been pointed at the funnel. `sweep` now sources through `for_source`, and Lita's *"Each investigator at your location gets +1 \[combat\]"* is the first granted modifier in the corpus. Two things the composition costs, both recorded on `sweep`: the walk is quadratic, since `for_source` runs its own board walk per source inside the walk over every source (the corpus is small, and every read is already a full re-derivation); and a grant's condition is now evaluated during a modifier read, which could in principle re-enter the sweep through `eval_condition` — it does not today, because both corpus grants condition on `Condition::ControlStatus`, which is board-global and reads no modified quantity. A grant conditioned on a *stat* is the case that would want a depth guard.

The registry reaches `for_source` differently from the other callers, and deliberately: a modified value is a pure query over `(state, registry)` and takes its registry **by argument**, so the funnel gained a registry-taking core (`for_source_with`, `location_abilities_with`) with the global-reading wrappers over it. One way to find a card's abilities, two ways to name the registry.
