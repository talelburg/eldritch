...
*ADR 0011 is otherwise untouched and is not superseded: anchors, surfaces, and the menu rule for selections all stand. One sentence in it is now stale and is corrected here rather than edited there — it names First Aid 01019's activated ability as an un-threaded dispatch site whose choices render in the banner, which predates #834's collapse of the two-field `EvalContext`; that ability has carried its `AbilitySource` since, and its choice anchors to its own asset.*
