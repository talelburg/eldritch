...
**The existing code does not yet match these rules.** All of `game-core`'s `engine/` is swept (#892, #893) and is the worked example to copy; outside it roughly 500 inline fully-qualified type paths remain in code bodies, all 6 surviving stacking-or-descending `super::` imports are in `state/`, and plain `use super::Item;` still stands in 14 files across seven crates. The rules bind new and modified code from now; #890's remaining sweep tickets bring the rest into line, per crate, and this paragraph goes when the last of them lands.
