...
**The existing code does not yet match these rules.** `game-core`'s `engine/dispatch/` is swept (#892) and is the worked example to copy; outside it roughly 1200 inline fully-qualified type paths and 7 stacking-or-descending `super::` imports remain. The rules bind new and modified code from now; #890's remaining sweep tickets bring the rest into line, per crate, and this paragraph goes when the last of them lands.
