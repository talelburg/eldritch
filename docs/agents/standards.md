...
**The existing code does not yet match these rules.** All of `game-core` is swept — `src/` by #892, #893 and #894, `tests/` by #895 — and all of `cards` by #896; both are the worked example to copy. Outside them roughly 120 inline fully-qualified type paths remain in code bodies — `web` 69, `scenarios` 36, `server` 10, `card-dsl` 7 — and plain `use super::Item;` still stands in 7 files across six crates. **No stacking-or-descending `super::` import survives anywhere in the workspace**; #894 cleared the last 6, which were `state/`'s. The rules bind new and modified code from now; #890's remaining sweep tickets bring the rest into line, per crate, and this paragraph goes when the last of them lands.
