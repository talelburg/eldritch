...
5. **One `use` per base path per scope.** `use crate::engine::dispatch::{cards, combat, cursor};`, not three lines — `combat.rs` carried seven consecutive `use crate::engine::dispatch::…;` before #892. `std` takes its own group at the top, separated by one blank line; everything below it is a single unbroken block, no blank lines inside.
6. **Reach an item through the module that defines it, not through a re-export** — `crate::engine::outcome::EngineOutcome`, `crate::engine::dispatch::emit::TimingEvent`, `evaluator::location_id_by_code` — unless the defining module is private and the re-export is the only way in (`crate::engine::Cx`; `engine::cx` is not `pub`). Two spellings of one item read as two items, and they collide the moment they meet: hoisting a function-body import in #892 produced `error[E0252]: the name OptionId is defined multiple times`, because the file already had it from `crate::engine::outcome` and the inner `use` took it from the `crate::engine` re-export.

**Ordering is `rustfmt`'s job; grouping and granularity are yours.** `reorder_imports` is on by default on stable and sorts both the statements in a group and the names inside a brace, so never hand-order either — that is also why `std` needs a blank line to stay on top. `imports_granularity` and `group_imports`, which would automate rule 5 entirely, are nightly-only and this repo pins stable.
...
**The existing code matches these rules, with two named exceptions.** Every crate is swept — `game-core` by #892, #893, #894 and #895, `cards` by #896, and `web`, `scenarios`, `server`, `card-dsl`, `card-data-pipeline` and `protocol` by #897 — so any file is a worked example to copy. **No inline fully-qualified *first-party* type path, no `use super::Item;`, and no stacking-or-descending `super::` import survives anywhere in the workspace.** The two exceptions are both #900's: rule 1's verb half over the card-effect DSL's builder functions, still imported by name and called bare (`seq(…)`, not `dsl::seq(…)`); and a handful of `std::` paths left inline where the bare leaf would read worse than the path — `impl std::fmt::Display` blocks, whose `fmt::Result` needs the collision hatch anyway (`scenario.rs`, `state/card.rs`, `card-data-pipeline`), and `std::collections::BTreeSet` in two `state/` field types. This sentence goes when #900 lands.
