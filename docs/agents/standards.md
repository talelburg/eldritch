...
**The existing code matches these rules, with one named exception.** Every crate is swept — `game-core` by #892, #893, #894 and #895, `cards` by #896, and `web`, `scenarios`, `server`, `card-dsl`, `card-data-pipeline` and `protocol` by #897 — so any file is a worked example to copy. **No inline fully-qualified type path, no `use super::Item;`, and no stacking-or-descending `super::` import survives anywhere in the workspace.** The exception is rule 1's verb half over the card-effect DSL's builder functions, which are still imported by name and called bare (`seq(…)`, not `dsl::seq(…)`): that is uniform, predates the standard, and #900 owns it as one pass — this sentence goes when it lands.
