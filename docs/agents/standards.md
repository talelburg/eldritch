...
4. **Scenario end-to-end tests** — `crates/scenarios/tests/`; the same per-binary process isolation, and reserved for tests that exercise **real scenario data in situ** — a Gathering walk, its agenda's doom cascade, its reference card's symbol tokens. The line is *does it drive scenario content*, not *is it a full walk*. A test that needs no scenario content belongs at layer 2 or 3. (The directory does not match this yet. The migration that empties it of synthetic-toy tests is tracked by [#864](https://github.com/talelburg/eldritch/issues/864), whose body holds the per-file disposition; when its last child lands, this parenthetical goes and the layer description stands alone.)
