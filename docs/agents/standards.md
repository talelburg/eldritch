...
| A card declaring `EventTiming::When` needs its triggering condition migrated to coordinator-owned resolution first — the per-condition migration, its cost, and its terminal condition | [ADR 0008](../adr/0008-a-triggering-condition-resolves-inside-its-own-sequence.md), and `ConditionResolution::Caller` in `crates/game-core/src/engine/dispatch/emit.rs` |
