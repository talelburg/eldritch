...
**The existing code does not yet match these rules.** All of `game-core`'s `src/` is swept (#892, #893, #894) and is the worked example to copy; outside it roughly 320 inline fully-qualified type paths remain in code bodies — `cards` 130, `web` 69, `game-core`'s own `tests/` 66, `scenarios` 36, `server` 10, `card-dsl` 7 — and plain `use super::Item;` still stands in 8 files across seven crates. **No stacking-or-descending `super::` import survives anywhere in the workspace**; #894 cleared the last 6, which were `state/`'s. The rules bind new and modified code from now; #890's remaining sweep tickets bring the rest into line, per crate, and this paragraph goes when the last of them lands.
