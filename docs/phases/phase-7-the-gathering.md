...
needed**. So the **EmitEvent-frame arc ([#435]) is the next objective**, with
#423 (Slice D) gated behind it — see Ordering step 6.
...
6. **EmitEvent-frame arc — the next objective.** Scoped + sliced in [`2026-06-22-emitevent-frame-arc-decomposition-design.md`](../superpowers/specs/2026-06-22-emitevent-frame-arc-decomposition-design.md); the named end-state from #393. Reify event emission, windows, and the timing matrix as `drive`-dispatched frames so the loop drives **every** frame. Four sequenced slices: **A** ([#433](https://github.com/talelburg/eldritch/issues/433)) window-taxonomy rework (`Resolution` → `FastWindow` + `TimingPointWindow{event, mode}`, drive-dispatched; the fast-window simplification) → **B** ([#434](https://github.com/talelburg/eldritch/issues/434)) `EmitEvent`/`TimingPoint` coordinators (the `when/at/after × forced/reaction` matrix; the one slice with new ordering behaviour) → **C** ([#431](https://github.com/talelburg/eldritch/issues/431)) loop-driven encounter-card disposal + `SkillTest` drive arm + retire the 5 synchronous re-entry sites → **D** ([#423](https://github.com/talelburg/eldritch/issues/423)) effect call-site migration off `apply_effect`. Umbrella for A+B is [#435](https://github.com/talelburg/eldritch/issues/435). Dependency order **A → B → C → D**; the §G Upkeep ordering pre-req shipped (PR #396). **Starting Slice A-i** (TimingPointWindow as a behaviour-preserving data swap). This supersedes the loose "#431 = the whole EmitEvent-frame slice" framing in the triage below.
