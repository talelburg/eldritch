...
- **A card effect initiates a skill test via `Effect::SkillTest { skill, difficulty, on_fail }`, with a margin-keyed failure branch via `Effect::ForEachPointFailed`; a Revelation that suspends discards through `GameState.pending_revelation_discard` (C4b prereq #286, PR #287).** `Effect::SkillTest` calls `start_skill_test` (kind `Plain`), always suspending at the commit window; `on_fail` (carried on `InFlightSkillTest.on_fail`, orthogonal to the success-side `follow_up`) runs on failure with the margin in `EvalContext::failed_by`. Because a test-initiating Revelation suspends, `resolve_encounter_card` records the treachery in `pending_revelation_discard`, flushed to `encounter_discard` at skill-test teardown (`None`/no-op for a plain Investigate). **C4c ([#235](https://github.com/talelburg/eldritch/issues/235)) extends `pending_revelation_discard`'s seam** for persistent treacheries that enter the threat area instead of discard. The two shared `Effect` variants cleared the two-consumer bar (3 / 2 cards); Crypt Chill's asset-discard and Ancient Evils' doom stayed card-local `Effect::Native` (single consumers), the latter via the new `pub place_doom_on_current_agenda`. Crypt Chill's interactive "choose an asset" is a deterministic first-asset stand-in (`TODO(#212)`). `Active`-status confirmed as in-play (not turn ownership), so Mythos-phase treachery tests are legal.
