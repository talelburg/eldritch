...
- [ ] **Step 5: Run the flip test + full game-core suite** — `RUSTFLAGS="-D warnings" cargo test -p game-core`. Fix any test still asserting open-turn `Done` (should be none after Task 4, but the `take_turn_action` helper's own callers now see `AwaitingInput` as the *returned* outcome — assert-on-effect tests are unaffected). Expected: PASS.
