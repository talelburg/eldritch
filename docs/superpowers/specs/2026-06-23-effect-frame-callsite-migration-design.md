...
- **Forced run is frame-driven (decided during implementation).** The forced-trigger path (`fire_forced_triggers` + callers `end_turn`, `fire_scenario_resolution`, the 8 forced test helpers) originally depended on the run completing **synchronously** — they branched on its `Done`/`AwaitingInput` boundary. Rather than keep a bounded synchronous driver, the forced run is made fully frame-driven (the #393 end-state): `resolve_one` / `fire_forced_triggers` **push** the forced effect frames and the global loop drives them; `end_turn` arms the existing `InvestigatorTurn { ending }` resumption before emitting `EndOfTurn`, so the loop rotates after the forced frames pop; the terminal `GameEnd` resolution hook (outside the main loop, holds the registry) and the 8 forced test helpers drive the pushed frames via the real `drive`. This removes the last production use of `drive_effect_to_base`, so it is deleted with `apply_effect`. The reverse-push in `fire_forced_triggers` preserves collection order for the (0/1-hit) loop; 2+ forced runs already route through the ordered forced-run frame.
